...
float* add_CFO (float *data0,long len_array,float ratio_offset){
    ...
    /*  scanf("%f",&ratio_offset);*/



      for (long index=0;index<len_sample;index++){

        sin_sample[index] = sin(-1*index*2*PI*ratio_offset);
        cos_sample[index] = cos(-1*index*2*PI*ratio_offset);
      }
